...
const _getDataModificationLogs = (req, tx, diff, beforeWrite) => {
  ...
  template.elements.forEach((ele, key) => {
    if (ele.picked.plain.categories.includes('DataSubjectID') && !diff[key] && !diff._old[key]) {
      diff._old[key] = req._?.partialPersistentState[0]?.[key]
    }
  })
  
  diff._old.lastName = req._.partialPersistentState[0].lastName
