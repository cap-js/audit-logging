...
const _applyTransition = (transition, data, d) => {
    ...
    if (v.transition) {
      if (Array.isArray(data[k])) {
        ...
        for (let i = 0; i < data[k].length; i++) {
          ...
          _applyTransition(v.transition, data[k][i], d[k][i])
        }
      } else {
        d[k] = { _op: data[k]._op }
        if (data[k]._old) d[k]._old = {}
        _applyTransition(v.transition, data[k], d[k])
      }
      continue
    } else if (v.ref) {
...
const _getDataWithAppliedTransitions = (data, req) => {
  ...
  if (transition) {
    d = { _op: data._op }
    if (data._old) d._old = {}
    _applyTransition(transition, data, d)
  }
  return d || data
}
