...
const _getDiffWithAppliedTransitions = (req, diff) => {
  ...
  if (transition) {
    diff._projection = { _op: diff._op }
    if (diff._old) diff._projection._old = {}
    for (const [k, v] of transition.mapping) {
      if (v.ref[0] in diff) diff._projection[k] = diff[v.ref[0]]
      if (diff._old && v.ref[0] in diff._old) diff._projection._old[k] = diff._old[v.ref[0]]
    }
  }
  return diff._projection || diff
}
...
const addDiffToCtx = async function (req) {
  ...
  diff = _getDiffWithAppliedTransitions(req, diff)

  // add keys, if necessary
  let keys = _getDiffWithAppliedTransitions(req, Object.assign({}, req.data))
