...
const addDiffToCtx = async function (req) {
  ...
  // Adding and removing @PersonalData.FieldSemantics: DataSubjectID from the record to ensure 
  // those values are fetched but not listed as attributes in log
  const keysToRemove = {}
  const firstDataRecord = req.data[0] ?? req.data;
  for (const ele in req.target.elements) {
    if (!firstDataRecord[ele] && req.target.elements[ele]['@PersonalData.FieldSemantics'] === 'DataSubjectID') {
      firstDataRecord[ele] = null;
      keysToRemove[ele] = 1;
    }
  }
  // get diff
  ...
  for (const ele in keysToRemove) {
    delete firstDataRecord[ele];
    delete diff[ele];
  }
