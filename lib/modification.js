    ...
    if (v.transition) {
      ...
      if (Array.isArray(data[ok])) {
        ...
        for (let i = 0; i < data[ok].length; i++) {
          d[k].push({ _op: data[ok][i]._op || d._op })
          if (old[ok]) d[k][i]._old = old[ok][i]
          _applyTransitionRecursively(v.transition, data[ok][i], d[k][i], old[ok]?.[i])
        }
        ...
        if (old[ok]) d[k]._old = old[ok]
        _applyTransitionRecursively(v.transition, data[ok], d[k], old[ok])
      }
    } else if (v.ref) {
