...
const hasPersonalData = (entity) => {
  if (entity.own($hasPersonalData) == null) {
    if (!entity["@PersonalData.EntitySemantics"])
      entity.set($hasPersonalData, false);
    else {
      ...
      if (
        ...
        entity["@PersonalData.DataSubjectRole"] = entity.name
          .match(/\w+/g)
          .pop();
      // prettier-ignore
...
const getPick = (event) => {
  return (element, target) => {
    ...
    if (element["@PersonalData.IsPotentiallySensitive"])
      categories.push("IsPotentiallySensitive");
  ...
  return keys
    ...
    .reduce((keys, key) => {
      ...
      keys.push({ ref: [alias, key.name] }, "=", {
        val: row[key.name] || row._old?.[key.name],
      });
      return keys;
    }, []);
};
...
const _alias = (entity) => {
  ...
  return entity.name
    .replace(`${entity._service.name}.`, "")
    .replace(/\./g, "_");
};

const _buildSubSelect = (
  model,
  { entity, relative, element, next },
  row,
  previousCqn,
) => {
  ...
  const childCqn = SELECT.from({ ref: [entityName], as }).columns(
    _keyColumns(keys, as),
  );
  ...
  // REVISIT: there seems to be a caching issue in cds^9 when elements are renamed
  if (!("_relations" in relative) || !relative._relations[element.name]) {
    const newRelation = Relation.to(relative);
    relative._relations = new Proxy(
      exposeRelation(newRelation),
      relationHandler(newRelation),
    );
  }
  ...
  if (previousCqn?._aliases.has(relativeAlias)) {
    ...
    for (const a in entity.associations)
      if (entity.associations[a].target === relative.name)
        t = entity.associations[a];
    if (t && w[0]?.xpr)
      for (const ele of w[0].xpr)
        if (ele.ref?.[0] === relativeAlias) ele.ref.splice(0, 1, as, t.name);
  }
  childCqn._aliases = new Set(
    previousCqn ? [...previousCqn._aliases.values(), as] : [as],
  );
...
const _getDataSubjectDown = (root, entity, prev, next) => {
  const associations = Object.values(entity.associations || {}).filter(
    (e) => !e._isBacklink,
  );
  ...
  for (const element of associations) {
    ...
    const me = { entity, relative: entity, element };
    if (prev) prev.next = me;
    const dataSubject = _getDataSubjectDown(
      root,
      element._target,
      me,
      next || me,
...
const getDataSubject = (entity, model) => {
  if (entity.own($dataSubject) == null) {
    ...
    if (!dataSubjectInfo)
      dataSubjectInfo = _getDataSubjectDown(entity.name, entity);
...
const _getDataSubjectsMap = (req) => {
  ...
  if (!_audit.dataSubjects.has(mapKey))
    _audit.dataSubjects.set(mapKey, new Map());
...
const addDataSubjectForDetailsEntity = (row, log, req, entity, model) => {
  ...
  const role =
    dataSubjectInfo.dataSubjectEntity["@PersonalData.DataSubjectRole"];
