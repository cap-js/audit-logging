...
const _getDataSubjectIdQuery = ({ dataSubjectEntity, subs }, row, model) => {
  ...
  const dsIDFields = Object.keys(dataSubjectEntity.elements).filter(ele => dataSubjectEntity.elements[ele]['@PersonalData.FieldSemantics'] === 'DataSubjectID').map(ele => dataSubjectEntity.elements[ele])

  const cqn = SELECT.one
    .from({ ref: [dataSubjectEntity.name], as })
    .columns(dsIDFields.length ? _keyColumns(dsIDFields, as) : _keyColumns(keys, as))
