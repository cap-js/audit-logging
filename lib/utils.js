...
const $dataSubject = Symbol('dataSubject')
const $parents = Symbol('parents')
const $visitedUp = Symbol('visitedUp')
const $visitedDown = Symbol('visitedDown')

const hasPersonalData = entity => {
  if (!entity.own($dataSubject)) {
    if (!entity['@PersonalData.EntitySemantics']) entity.set($dataSubject, false)
    else {
      ...
      entity.set($dataSubject, hasPersonalData)
    }
  }
