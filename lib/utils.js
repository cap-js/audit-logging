...
const hasPersonalData = entity => {
  if (entity.own($hasPersonalData) == null) {
    if (!entity['@PersonalData.EntitySemantics']) entity.set($hasPersonalData, false)
      ...
      // if (hasPersonalData && entity.name.startsWith('CollaborationsService.')) debugger
...
const getPick = event => {
  return (element, target) => {
    // if (target.name === 'CollaborationsService.Collaborations') debugger
...
const _buildSubSelect = (model, { entity, relative, element, next }, row, previousCqn) => {
  ...
  // REVISIT: rewrite to path expression, if alias for relative is already used in subselect
...
const _getDataSubjectUp = (root, model, entity, prev, next, result) => {
  ...
  const __dss = _ups.filter(e => e.parent['@PersonalData.EntitySemantics'] === 'DataSubject')
  for (const element of _ups) {
    ...
    if (element.parent['@PersonalData.EntitySemantics'] === 'DataSubject') {
      ...
      // REVISIT: with dfs alone, we don't find the shortest path
      // dfs is a must here
