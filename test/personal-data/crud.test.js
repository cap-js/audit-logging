...
describe("personal data audit logging in CRUD", () => {
  ...
  describe("data access logging", () => {
    test("read with another data subject and sensitive data only in composition children", async () => {
      const { data: customer } = await GET(`/crud-2/Customers(${CUSTOMER_ID})?$expand=addresses`, {
        auth: ALICE
      });
    ...
    test("wrongly modeled entity must not be logged", async () => {
      const response = await GET(`/crud-2/Customers(${CUSTOMER_ID})?$expand=status,addresses`, {
        auth: ALICE
      });
  ...
  describe("modification logging", () => {
    ...
    test("update Customer - deep", async () => {
      ...
      response = await GET(`/crud-1/Customers(${CUSTOMER_ID})?$expand=addresses,status`, {
        auth: ALICE
      });
      ...
      if (!response.data.addresses) {
        ...
        } = await GET(`/crud-1/Customers(${CUSTOMER_ID})?$select=ID&$expand=addresses,status`, {
          auth: ALICE
        });
    ...
    test(`with entity semantics -Other- and downward lookup of data subject ID`, async () => {
      ...
      } = await GET(`/crud-1/Orders(${order.ID})?$expand=header($expand=sensitiveData),items`, {
        auth: ALICE
      });
      items.push({
        name: "bar",
  ...
  describe("with renamings", () => {
    test("one level", async () => {
      ...
      await DELETE(`/crud-3/R1/${r1.r1_ID}`, { auth: ALICE });
      expect(_logs.length).toBe(1);
      expect(_logs).toContainMatchObject({
        user: "alice",
        object,
    ...
    test("two levels", async () => {
      ...
      await DELETE(`/crud-3/R2/${r2.r2_ID}`, { auth: ALICE });
      expect(_logs.length).toBe(1);
      expect(_logs).toContainMatchObject({
        user: "alice",
        object,
