...
describe("personal data audit logging in CRUD", () => {
  ...
  describe("modification logging", () => {
    ...
    test("arrayed property", async () => {
      ...
      expect(_logs.length).toBe(2);
      expect(_logs[0].attributes).toEqual([
  ...
  test("entity name alias works with annotations on database level if association target is not published in service", async () => {
    const response = await POST(
      "/crud-6/D",
      {
        dataSubject_ID: "17609000-3860-415b-b007-1b59dae2a198",
        text: "I am personal",
      },
      { auth: ALICE },
    );
    ...
    expect(_logs[0]).toMatchObject({
      data_subject: {
        role: "C",
        type: "sap.auditlog.test.personal_data.db.C",
      },
      object: {
        type: "CRUD_6.D",
