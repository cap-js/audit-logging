...
describe("personal data audit logging in Fiori", () => {
  ...
  describe("data access logging for active draft enabled entities", () => {
    ...
    test("read single Customer", async () => {
      const response = await GET(`/fiori-1/Customers(ID=${CUSTOMER_ID},IsActiveEntity=true)`, {
        auth: ALICE
      });
    ...
    test("read all CustomerPostalAddress expanding Customer", async () => {
      const response = await GET("/fiori-1/CustomerPostalAddress?$expand=customer", {
        auth: ALICE
      });
  ...
  describe("modification and read draft logging", () => {
    test("draft edit, patch and activate with another data subject and sensitive data only in composition children", async () => {
      ...
      await GET(`/fiori-2/Customers(ID=${CUSTOMER_ID},IsActiveEntity=false)?$expand=addresses`, {
        auth: ALICE
      });
    ...
    test("create, patch, read and activate", async () => {
      ...
      response = await GET(`/fiori-1/Customers(ID=${customer.ID},IsActiveEntity=false)`, {
        auth: ALICE
      });
    ...
    test("draft edit, read union, delete draft", async () => {
      ...
      response = await DELETE(`/fiori-1/Customers(ID=${CUSTOMER_ID},IsActiveEntity=false)`, {
        auth: ALICE
      });
    ...
    test("delete active Customer - deep", async () => {
      ...
      response = await DELETE(`/fiori-1/Customers(ID=${CUSTOMER_ID},IsActiveEntity=true)`, {
        auth: ALICE
      });
